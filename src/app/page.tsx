...
const usePurlinComputations = ({
  ...
  const typeOfSection = typeOfSections.find(
    ([tableName]) => tableName === typeOfSectionName
  );
  invariant(typeOfSection, "Invalid type of section");

  const getLoadOnPurlin = () => {
    return Decimal.add(weightOfTiles, liveLoad)
      .mul(rangeOfPurlin)
      .add(useSelfWeight)
      .toNumber();
  };
  ...
  const getMomentOfMx = () => {
    // =(1/8)*(I11^2)*I21
    return Decimal.mul(Decimal.pow(spanLength, 2), getUnifLoadOfWy())
      .div(8)
      .toNumber();
  };
  ...
  const getMomentOfMy = () => {
    ...
    return Decimal.mul(typeOfSagRodConst, Decimal.pow(spanLength, 2))
      .mul(getUnifLoadOfWx())
      .toNumber();
  };
  ...
  const getReqSectModulus = () => {
    // =100*MAX(I22,I23)/N13
    return Decimal.mul(100, Decimal.max(getMomentOfMx(), getMomentOfMy()))
      .div(getAllBendStress())
      .toNumber();
  };
  ...
  const getActualDeflextion = () => {
    ...
    return Decimal.div(
      getDeflextion(),
      Decimal.mul(modulusOfElasticity, 100).mul(
        Decimal.max(getMOfIn(), getIy())
      )
    ).toNumber();
  };
  ...
  const getValidatedActualDeflextion = () => {
    const calculatedDeflextion = Decimal.mul(spanLength, 100)
      .div(+allowableDeflection)
      .toNumber();
...
const PropertiesOfSteelForDesign = ({
  form,
}: {
  form: UseFormReturn<z.infer<typeof formSchema>>;
}) => {
  const { getUseSteelGrade, getUltimateStrength, getAllBendStress } =
    usePurlinComputations({ form });

  return (
    <div className="space-y-2">
      <h2 className="text-2xl font-bold mb-4">
        Part II: Properties Of Steel For Design
      </h2>
      <div>
...
const SelectTypeAndSectionOfSteelForm = ({
  form,
}: {
  form: UseFormReturn<z.infer<typeof formSchema>>;
}) => {
...
const RecheckAllowableStressOnSection = ({
  form,
}: {
  form: UseFormReturn<z.infer<typeof formSchema>>;
}) => {
  ...
  return (
    <div className="space-y-2">
      ...
      <div>
        ...
        {getValidatedActualBendingStress().status === "success" ? (
          <FormMessage className="text-green-500">
            {getValidatedActualBendingStress().message}
          </FormMessage>
        ) : (
      ...
      <div>
        ...
        {getValidatedActualDeflextion().status === "success" ? (
          <FormMessage className="text-green-500">
            {getValidatedActualDeflextion().message}
          </FormMessage>
        ) : (
          <FormMessage className="text-red-500">
            {getValidatedActualDeflextion().message}
      ...
      <div>
        ...
        {getValidatedActualSelfWeight().status === "success" ? (
          <FormMessage className="text-green-500">
            {getValidatedActualSelfWeight().message}
          </FormMessage>
        ) : (
          <FormMessage className="text-red-500">
            {getValidatedActualSelfWeight().message}
      ...
      <div>
        ...
        {getValidatedSectModulus().status === "success" ? (
          <FormMessage className="text-green-500">
            {getValidatedSectModulus().message}
          </FormMessage>
        ) : (
          <FormMessage className="text-red-500">
            {getValidatedSectModulus().message}
